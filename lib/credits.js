...
async function grantDaily(userId, role) {
  ...
  try {
    ...
    await client.query(
      `INSERT INTO daily_free (user_id, day, granted) VALUES ($1, ${DAY}, 0)
       ON CONFLICT (user_id, day) DO NOTHING`,
      [userId]
    );
    await client.query(
      `SELECT 1 FROM daily_free WHERE user_id = $1 AND day = ${DAY} FOR UPDATE`,
      [userId]
    );
    const { rows } = await client.query(
      `SELECT COALESCE(SUM(granted), 0) AS g FROM daily_free WHERE user_id = $1 AND day >= ${DAY}`,
      [userId]
    );
    ...
    if (missing > 0) {
      ...
      await client.query(
        `UPDATE daily_free SET granted = granted + $1 WHERE user_id = $2 AND day = ${DAY}`,
        [missing, userId]
      );
    }
